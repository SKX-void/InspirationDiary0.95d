...
class DocSys:
    ...
    @staticmethod
    def preprocess_delta_ops_for_header(ops) -> []:
        """
        ...
        while i < len(ops):
            ...
            has_special = has_header or has_list or has_align or has_indent or has_code_block or has_blockquote
    ...
    def write_delta_run(delta_ops, doc):
        ...
        preprocess_ops = DocSys.preprocess_delta_ops_for_header(delta_ops.get('ops', []))
        ...
        def set_paragraph_style(paragraph, style):
            def convert_to_code_block(to_paragraph, background_color="fdf6e3"):
                ...
                """
                doc_handle = to_paragraph._parent
                original_text = to_paragraph.text
                to_paragraph.clear()

                # 在原段落位置插入一个单行单列的表格（注意这里加了 width 参数）
                table = doc_handle.add_table(rows=1, cols=1, width=Cm(15))  # 表格宽度设为 15 厘米
                cell = table.cell(0, 0)
                ...
                run._element.rPr.rFonts.set(qn('w:eastAsia'), 'Courier New')  # 中文兼容
                run.font.size = Pt(10)

                # 删除原始段落（现在它是空的）
                p = to_paragraph._element
                p.getparent().remove(p)

            def convert_to_blockquote(paragraph, background_color="E6E6E6", left_border_color="4472C4"):
                """
                ...
                if isinstance(insert, str):
                    ...
                    for i, line in enumerate(lines):
                        if line == '\n':
                            ...
                            current_paragraph = process_accumulated(line, current_paragraph, op.get('attributes', {}))
                    ...
                    if 'image' in insert:
                        ...
                        if image_data.startswith('data:image'):
                            ...
                            run.add_picture('temp_image.png', width=Inches(2))
    ...
    def apply_attributes(run, attrs):
        ...
        if 'underline' in attrs:
            run.underline = WD_UNDERLINE.SINGLE if attrs['underline'] is True else WD_UNDERLINE.DOUBLE
        ...
        if 'font' in attrs:
            ...
            run._element.rPr.rFonts.set(qn('w:eastAsia'), DocSys.FONT_MAP['default'])
        if 'size' in attrs:
...
def add_toc(doc):
    ...
    fld_char = OxmlElement('w:fldChar')  # 创建字段字符
...
def main():
    ...
    parser.add_argument('output_path', type=str, help='Path to save the generated DOCX file.')
    parser.add_argument('--generate-toc', action='store_true', help='Generate a table of contents.')
