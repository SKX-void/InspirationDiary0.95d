...
class HistoryDom {
    ...
    static async loadHistoryDetail(historyId: number) {
        ...
        if (item) {
            ...
            if (!(detailId instanceof HTMLElement) || !(detailChapter instanceof HTMLElement) || !(detailPage instanceof HTMLElement) || !(detailTime instanceof HTMLElement)) {
                return;
            }
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!(modalClose instanceof HTMLElement)) {
        console.error('modal-close元素加载失败');
        return;
    }
    ...
    if (!(refreshBtn instanceof HTMLElement)) {
        console.error('refresh-btn元素加载失败');
        return;
    }
    // 刷新按钮
